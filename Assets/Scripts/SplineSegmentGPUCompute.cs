...
namespace PS03
{
    ...
    public class SplineSegmentGPUCompute : MonoBehaviour
    {
        ...
        [SerializeField] private LineRenderer firstDerivative1, firstDerivative2;     //to draw tangents
        [SerializeField] private LineRenderer secondDerivative1, secondDerivative2;    //to draw normals

        // what color should the Spline Curve be?
        [SerializeField] private Color splineColor = new Color(255f / 255f, 255f / 255f, 255f / 255f);
        ...
        {
            toDisplay = !toDisplay;
            firstDerivative1.enabled = toDisplay;
            firstDerivative2.enabled = toDisplay;
            secondDerivative1.enabled = toDisplay;
            secondDerivative2.enabled = toDisplay;
        }

        public void SetType(SplineParameters.SplineType type)
        {
            splineType = type;
            Debug.Log(splineType);
        }
        ...
        private void Awake()
        {
            ...
            material = new Material(Shader.Find("SplineVertexShader"));
        ...
        private void Update()
        {
            ...
            //Derivative's calculation: 
            //Vector3 tangent1 = 3 * (control1.position - control0.position);
            //Vector3 tangent2 = 3 * (control3.position - control2.position);
            //Vector3 normal1 = 6 * (control0.position - (2 * control1.position) + control2.position);
            //Vector3 normal2 = 6 * (control2.position - (2 * control3.position) + control3.position);
            Vector3 tangent1;
            Vector3 tangent2;
            Vector3 normal1;
            Vector3 normal2;
            ...
            {
                tangent1 = 3 * (control1.position - control0.position);
                tangent2 = -3 * (control3.position - control2.position);        //negative so that it will point in the same direction as the derivative from the first control point
                normal1 = 6 * (control0.position - (2 * control1.position) + control2.position);
                normal2 = 6 * (control1.position - (2 * control2.position) + control3.position);

                // to draw the tangents and normals:
                //firstDerivatives.SetPosition(0, control0.position + tangent1);
                firstDerivative1.SetPosition(0, control0.position);
                firstDerivative1.SetPosition(1, (control0.position + tangent1.normalized));
                firstDerivative2.SetPosition(0, control3.position);
                firstDerivative2.SetPosition(1, control3.position + tangent2.normalized);
                secondDerivative1.SetPosition(0, control0.position);
                secondDerivative1.SetPosition(1, control0.position + normal1.normalized);
                secondDerivative2.SetPosition(0, control3.position);
                secondDerivative2.SetPosition(1, control3.position + normal2.normalized);

            }
            else if (splineType == SplineParameters.SplineType.CatmullRom)
            {
                tangent1 = 0.5f * (control2.position - control0.position);
                tangent2 = -0.5f * (control3.position - control1.position);         //negative so that it will point in the same direction as the derivative from the first control point
                normal1 = (2 * control0.position) - (5 * control1.position) + (4 * control2.position) - control3.position;
                normal2 = (-control0.position + (4 * control1.position) - (5 * control2.position) + (2 * control3.position));

                // to draw the tangents and normals:
                //firstDerivatives.SetPosition(0, control0.position + tangent1);
                firstDerivative1.SetPosition(0, control1.position);
                firstDerivative1.SetPosition(1, control1.position + tangent1.normalized);
                firstDerivative2.SetPosition(0, control2.position);
                firstDerivative2.SetPosition(1, control2.position + tangent2.normalized);
                secondDerivative1.SetPosition(0, control1.position);
                secondDerivative1.SetPosition(1, control1.position + normal1.normalized);
                secondDerivative2.SetPosition(0, control2.position);
                secondDerivative2.SetPosition(1, control2.position + normal2.normalized);  
            }
            else
            {
                tangent1 = 0.5f * (control2.position - control0.position);
                tangent2 = -0.5f * (control3.position - control1.position);      //negative so that it will point in the same direction as the derivative from the first control point
                normal1 = (control0.position - (2 * control1.position) + control2.position);
                normal2 = (control1.position - (2 * control2.position) + control3.position);
